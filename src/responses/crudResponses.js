...
module.exports = {
    ...
    createResponse: (res, message = 'Item created successfully', status_code = 200) => {
        res.status(status_code).send({ message })
    },

    browseResponse: (req, res, data, message = 'Data fetched successfully', statuse_code = 200) => {
        res.status(statuse_code).send({
            message,
            data: data.rows,
            meta: {
    ...
    viewResponse: (res, data, message = 'Item fetched successfully', status_code = 200) => {
        res.status(status_code).send({ message, data })
    },

    updateResponse: (res, message = 'Item updated successfully', status_code = 200) => {
        res.status(status_code).send({ message })
    },

    errorResponse: (res, errors = [], status_code = 500) => {
        res.status(status_code).send({ message: HTTP_STATUS_CODES[`CODE_${status_code}`], errors });
    },

    deleteResponse: (res, message = 'Item deleted successfully', status_code = 200) => {
        res.status(status_code).send({ message });
    },
}